...
class Schedules extends CI_Model {
    ...
    */
    ...
    function repeat_post_until($user_id, $user_gmt_zone)
    {
        ...
        foreach($qjobs->result() as $post)
        {
            ...
            $qposts = $this->db->query("SELECT id FROM cronjobs WHERE id_post='".$post->id."' ORDER BY id DESC");
            $interval = 0;
            foreach($qposts->result() as $posts)
            {
                if(!empty($field_interval)) $interval += $field_interval;
