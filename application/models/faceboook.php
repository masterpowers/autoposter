...
class Faceboook extends CI_Model {
    ...
    function fbdata($user_id, $action = false, $groups = false)
    {
        ...
        if($qfbdata->num_rows() > 0)
        {
            if($action == 'update')
            {
                ...
                if(in_array($this->app_version, array('v2.4', 'v2.5')) && empty($groups))
                {
    ...
    function check_token_permissions()
    {
        ...
        if(!empty($permissions) && is_array($permissions) && !isset($permissions['error']))
        {
            ...
            if(in_array($this->app_version, array('v2.4', 'v2.5')))
            {
        ...
        if(isset($_POST))
        {
            ...
            if(!empty($file))
            {
                ...
                if(isset($matches[1]) && isset($matches[2]) && isset($matches[3]))
                {
                    ...
                    {
                        if((isset($matches[3][$key]) && $matches[3][$key] == 'null,') || $this->groups_limit == $i) break;
